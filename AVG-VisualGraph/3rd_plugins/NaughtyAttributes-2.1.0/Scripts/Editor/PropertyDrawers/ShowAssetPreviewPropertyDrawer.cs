    ...
    public class ShowAssetPreviewPropertyDrawer : PropertyDrawerBase
    {
        protected override float GetPropertyHeight_Internal(SerializedProperty property, GUIContent label)
        {
            if (property.propertyType == SerializedPropertyType.ObjectReference)
            {
                ...
                if (previewTexture != null)
                {
                    ...
                    if (showAssetPreviewAttribute.IsShowLable)
                        return GetPropertyHeight(property) + GetAssetPreviewSize(property).y;
                    else return GetPropertyHeight(property) + GetAssetPreviewSize(property).y - EditorGUIUtility.singleLineHeight;
                }
                else
                {
        ...
        protected override void OnGUI_Internal(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            if (property.propertyType == SerializedPropertyType.ObjectReference)
            {
                ...
                //CHANGES:改变了NaughtyAttributes.ShowAssetPreviewAttribute
                ShowAssetPreviewAttribute showAssetPreviewAttribute = PropertyUtility.GetAttribute<ShowAssetPreviewAttribute>(property);
                if (showAssetPreviewAttribute.IsShowLable)
                    EditorGUI.PropertyField(propertyRect, property, label);
                if (showAssetPreviewAttribute.IsShowProperty) EditorGUI.PropertyField(propertyRect, property);
                ...
                if (previewTexture != null)
                {
                    ...
                    {
                        x = rect.x + NaughtyEditorGUI.GetIndentLength(rect) + 150,
        ...
        private Vector2 GetAssetPreviewSize(SerializedProperty property)
        {
            ...
            else
            {
                ...
                //if (!showAssetPreviewAttribute.IsShowLable)
                //{
                //    height -= Mathf.RoundToInt(EditorGUIUtility.singleLineHeight);
                //}
